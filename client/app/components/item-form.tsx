...
import { seriesApi, type Series, type Book, type Item } from "@/lib/api";
...
interface ItemFormProps {
  ...
  initialData?: Partial<Item> & { bookIds?: string[] };
...
export function ItemForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!name?.trim()) {
      setInternalError("Item name is required");
      return;
    }
      ...
      {selectedSeriesId && booksList.length > 0 && (
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Select which books this item appears in
          </p>
        </div>
      )}
